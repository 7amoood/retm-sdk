...
class Worker
{
    ...
    public function daemon($connectionName, $queue, WorkerOptions $options)
    {
        ...
        while (true) {
            ...
            if ($job) {
                $jobsProcessed++;
                if ($job->payload()['business'] ?? null) try {
                    $appRunning = app('APP');
                    if ($job->payload()['business'] != $appRunning['REFRENCE']) {
                        app()->instance('APP', include(app('path.core') . '../SERVICES/' . $job->payload()['service'] . '/CLIENTS/' . $job->payload()['business'] . '/connection.php'));
                    }
                } catch (\Exception $e) {
                    app()->instance('APP', include(app('path.core') . '../SERVICES/' . $job->payload()['service'] . '/CLIENTS/' . $job->payload()['business'] . '/connection.php'));
                }
